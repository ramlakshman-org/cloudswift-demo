...
- **Admin:** `/admin` — password-protected dashboard for triaging enquiries by category/status, with structured answers from `/assessment` submissions shown inline
...
| `ADMIN_PASSWORD` | Shared password for `/admin` login. Required for the admin dashboard to be reachable at all — without it, login always fails. |
