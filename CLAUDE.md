...
- **Cal.com booking flow uses a placeholder slug.** `src/lib/cal.ts` has `CAL_LINK = "cloudswift/cloud-assessment"` — not a real event yet. Every "Book Your Free Assessment" CTA sitewide and the `/assessment` page's embed point at this. Once a real Cal.com account + event type ("Free Cloud Cost & Risk Assessment", 30 min) is created, update that one constant — nothing else needs to change. Theme/brand color are already wired (`CAL_UI_CONFIG` in the same file, light theme, `#e05c20` rust to match the site's primary CTA color).
